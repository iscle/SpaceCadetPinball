...
void render::init(gdrv_bitmap8 *bmp, float zMin, float zScaler, int width, int height) {
    ...
    {
        UsingSdlHint hint{SDL_HINT_RENDER_SCALE_QUALITY,
                          options::Options.LinearFiltering ? "linear" : "nearest"};
...
void render::update() {
    ...
    for (auto curSprite : dirty_list) {
        ...
        switch (curSprite->VisualType) {
            case VisualTypes::Sprite:
                if (curSprite->DirtyRectPrev.Width > 0)
                    maths::enclosing_box(&curSprite->DirtyRectPrev, &curSprite->BmpRect,
                                         &curSprite->DirtyRect);

                if (maths::rectangle_clip(&curSprite->DirtyRect, &vscreen_rect,
                                          &curSprite->DirtyRect))
                    clearSprite = true;
                else
            ...
            case VisualTypes::None:
                if (maths::rectangle_clip(&curSprite->BmpRect, &vscreen_rect,
                                          &curSprite->DirtyRect))
        ...
        if (clearSprite) {
            ...
            if (background_bitmap)
                gdrv::copy_bitmap(vscreen, width, height, xPos, yPos, background_bitmap, xPos,
                                  yPos);
            else
    ...
    for (auto sprite : dirty_list) {
        if (sprite->DirtyRect.Width > 0 &&
            (sprite->VisualType == VisualTypes::None || sprite->VisualType ==
                                                        VisualTypes::Sprite))
            repaint(sprite);
    }
...
render::create_sprite(VisualTypes visualType, gdrv_bitmap8 *bmp, zmap_header_type *zMap,
...
void
render::sprite_set(render_sprite_type_struct *sprite, gdrv_bitmap8 *bmp, zmap_header_type *zMap,
                   int xPos,
                   int yPos) {
    if (sprite) {
...
void render::ball_set(render_sprite_type_struct *sprite, gdrv_bitmap8 *bmp, float depth, int xPos,
                      int yPos) {
    if (sprite) {
        sprite->Bmp = bmp;
        if (bmp) {
...
void render::paint_balls() {
    ...
    for (auto index = 0u; index < ball_list.size(); ++index) {
        ...
        if (ball->Bmp && maths::rectangle_clip(&ball->BmpRect, &vscreen_rect, &ball->DirtyRect)) {
            ...
            gdrv::copy_bitmap(ball_bitmap[index], dirty->Width, dirty->Height, 0, 0, vscreen, xPos,
                              yPos);
            zdrv::paint_flat(
                    dirty->Width,
                    dirty->Height,
        ...
        if (!mainSprite->UnknownFlag && mainSprite->BoundingRect.Width != -1) {
            ...
            for (auto refSprite : sprite_list) {
                if (!refSprite->UnknownFlag
                    ...
                    && maths::rectangle_clip(&mainSprite->BoundingRect, &refSprite->BoundingRect,
                                             nullptr)
                    && spriteArr) {
...
void render::BlitVScreen() {
    ...
    assertm(static_cast<unsigned>(pitch) == vscreen->Width * sizeof(ColorRgba),
            "Padding on vScreen texture");

    if (offset_x == 0 && offset_y == 0) {
        // No offset - direct copy
        std::memcpy(lockedPixels, vscreen->BmpBufPtr1,
                    vscreen->Width * vscreen->Height * sizeof(ColorRgba));
    } else {
