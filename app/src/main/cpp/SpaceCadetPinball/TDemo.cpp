...
TDemo::TDemo(TPinballTable *table, int groupIndex)
    ...
    Edge3 = TEdgeSegment::install_wall(v9, this, &ActiveFlag, visual.CollisionGroup,
                                       table->CollisionCompOffset, 1404);
}
...
void TDemo::Collision(TBall *ball, vector_type *nextPosition, vector_type *direction, float coef,
                      TEdgeSegment *edge) {
    ball->not_again(edge);
    ...
    switch (reinterpret_cast<size_t>(edge->WallValue)) {
        case 1400:
            if (!FlipLeftTimer && !FlipLeftFlag) {
                float time = FlipTimerTime1 + FlipTimerTime2 -
                             RandFloat() * (FlipTimerTime2 + FlipTimerTime2);
