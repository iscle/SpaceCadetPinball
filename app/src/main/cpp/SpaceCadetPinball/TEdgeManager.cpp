...
int TEdgeManager::TestGridBox(int x, int y, float *distPtr, TEdgeSegment **edgeDst, ray_type *ray,
                              TBall *ball,
                              int edgeIndex) {
    if (x >= 0 && x < 10 && y >= 0 && y < 15) {
        ...
        for (auto it = edgeBox->EdgeList.rbegin(); it != edgeBox->EdgeList.rend(); ++it) {
            auto edge = *it;
            if (!edge->ProcessedFlag && *edge->ActiveFlag &&
                (edge->CollisionGroup & ray->FieldFlag)) {
