...
std::vector<uint8_t> *midi::MdsToMidi(std::string file) {
    ...
    do {
        ...
        auto dataChunk = reinterpret_cast<riff_data *>(
                reinterpret_cast<char *>(&fileBuf->dwTimeFormat) + fileBuf->
                        FmtSize);
        ...
        std::sort(midiEvents.begin(), midiEvents.end(),
                  [](const midi_event &lhs, const midi_event &rhs) {
        ...
        // Set final MTrk size
        auto lengthBE = SwapByteOrderInt(
                static_cast<uint32_t>(midiBytes.size()) - sizeof header - sizeof track);
