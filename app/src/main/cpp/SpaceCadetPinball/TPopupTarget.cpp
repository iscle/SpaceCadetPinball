...
TPopupTarget::TPopupTarget(TPinballTable *table, int groupIndex) : TCollisionComponent(table,
                                                                                       groupIndex,
                                                                                       true) {
    this->Timer = 0;
...
void
TPopupTarget::Collision(TBall *ball, vector_type *nextPosition, vector_type *direction, float coef,
                        TEdgeSegment *edge) {
    if (this->PinballTable->TiltLockFlag) {
        maths::basic_collision(ball, nextPosition, direction, this->Elasticity, this->Smoothness,
                               1000000000.0, 0.0);
