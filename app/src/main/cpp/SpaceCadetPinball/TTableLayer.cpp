...
TTableLayer::TTableLayer(TPinballTable *table) : TCollisionComponent(table, -1, false) {
    ...
    GraityDirX = cos(PinballTable->GravityAnglY) * sin(PinballTable->GravityAngleX) *
                 PinballTable->GravityDirVectMult;
    GraityDirY = sin(PinballTable->GravityAnglY) * sin(PinballTable->GravityAngleX) *
                 PinballTable->GravityDirVectMult;
...
void TTableLayer::edges_insert_circle(circle_type *circle, TEdgeSegment *edge,
                                      field_effect_type *field) {
    ray_type ray{};
    vector_type vec1{};
