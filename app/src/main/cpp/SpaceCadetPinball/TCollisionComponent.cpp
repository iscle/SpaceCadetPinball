...
TCollisionComponent::TCollisionComponent(TPinballTable *table, int groupIndex, bool createWall) :
    ...
    if (groupIndex <= 0) {
        ...
        if (createWall) {
            ...
            TEdgeSegment::install_wall(floatArr, this, &ActiveFlag, visual.CollisionGroup, offset,
                                       0);
        }
    }
...
int TCollisionComponent::DefaultCollision(TBall *ball, vector_type *nextPosition,
    ...
    auto projSpeed = maths::basic_collision(ball, nextPosition, direction, Elasticity, Smoothness,
                                            Threshold, Boost);
