...
int maths::overlapping_box(rectangle_type *rect1, rectangle_type *rect2, rectangle_type *dstRect) {
    ...
    return dstRect->Width <= rect2->Width + rect1->Width &&
           dstRect->Height <= rect2->Height + rect1->Height;
}
...
float maths::ray_intersect_line(ray_type *ray, line_type *line) {
    ...
    float perpDot =
            line->PerpendicularL.Y * ray->Direction.Y + ray->Direction.X * line->PerpendicularL.X;
    if (perpDot < 0.0f) {
        float result = -(
                (ray->Origin.X * line->PerpendicularL.X + ray->Origin.Y * line->PerpendicularL.Y +
                 line->
                         PreComp1)
                / perpDot);
...
float maths::basic_collision(TBall *ball, vector_type *nextPosition, vector_type *direction,
...
void maths::find_closest_edge(ramp_plane_type *plane, int planeCount, wall_point_type *wall,
                              vector_type **lineEnd,
