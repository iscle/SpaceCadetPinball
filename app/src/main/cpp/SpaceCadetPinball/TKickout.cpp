...
void TKickout::Collision(TBall *ball, vector_type *nextPosition, vector_type *direction, float coef,
                         TEdgeSegment *edge) {
    if (!KickFlag1) {
        Ball = ball;
...
void TKickout::TimerExpired(int timerId, void *caller) {
    ...
    if (kick->KickFlag1) {
        ...
        if (kick->Ball) {
            ...
            TBall::throw_ball(kick->Ball, &kick->BallAcceleration, kick->ThrowAngleMult,
                              kick->ThrowSpeedMult1,
