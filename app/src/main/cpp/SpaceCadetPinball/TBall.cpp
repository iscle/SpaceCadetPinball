...
TBall::TBall(TPinballTable *table) : TPinballComponent(table, -1, false) {
    ...
    if (visualCount > 0) {
        ...
        do {
            ...
            auto visVec = reinterpret_cast<vector_type *>(loader::query_float_attribute(groupIndex,
                                                                                        index,
                                                                                        501));
...
void TBall::throw_ball(TBall *ball, vector_type *acceleration, float angleMult, float speedMult1,
                       float speedMult2) {
