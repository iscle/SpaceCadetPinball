...
void TRamp::Collision(TBall *ball, vector_type *nextPosition, vector_type *direction, float coef,
                      TEdgeSegment *edge) {
    ball->not_again(edge);
    ...
    if (plane) {
        ...
        ball->Position.Z = ball->Position.X * ball->CollisionOffset.X +
                           ball->Position.Y * ball->CollisionOffset.Y +
